...
import binary_decision_functions
import patterns
        ...
        if len(self.categorical) == 0:
            for i in range(len(test)):
                for j in range(len(train_pos)):
                    ...
                    positive_support[i][j] = sum(
                        (~((intsec.low <= train_pos) * (train_pos <= intsec.high))).sum(axis=1) == 0)
                    positive_counter[i][j] = sum(
                        (~((intsec.low <= train_neg) * (train_neg <= intsec.high))).sum(axis=1) == 0)

                for j in range(len(train_neg)):
                    intsec = patterns.IntervalPattern(test[i], train_neg[j])
                    negative_support[i][j] = sum(
                        (~((intsec.low <= train_neg) * (train_neg <= intsec.high))).sum(axis=1) == 0)
                    negative_counter[i][j] = sum(
                        (~((intsec.low <= train_pos) * (train_pos <= intsec.high))).sum(axis=1) == 0)
            ...
            for i in range(len(test)):
                for j in range(len(train_pos)):
                    ...
                    positive_support[i][j] = sum(
                        ((~((intsec_num.low <= train_pos_num) * (train_pos_num <= intsec_num.high))).sum(axis=1) == 0) *
                        ((~(train_pos_cat[:, intsec_cat.mask] == intsec_cat.vals)).sum(axis=1) == 0))
                    positive_counter[i][j] = sum(
                        ((~((intsec_num.low <= train_neg_num) * (train_neg_num <= intsec_num.high))).sum(axis=1) == 0) *
